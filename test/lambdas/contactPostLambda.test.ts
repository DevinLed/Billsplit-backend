...
jest.mock('@aws-sdk/client-cognito-identity-provider', () => ({
  CognitoIdentityProvider: jest.fn().mockImplementation(() => ({
    listUsers: jest.fn().mockImplementation((command) => {
      const users = [
        { Username: 'user123', Attributes: [{ Name: 'email', Value: 'existing@example.com' }] },
        { Username: 'currentUser123', Attributes: [{ Name: 'email', Value: 'current@example.com' }] },
      ];
      const filteredUsers = users.filter(user => command.Filter.includes(user.Attributes[0].Value));
...
describe('postContactHandler Tests', () => {
  ...
  test('Should create contact and send notification successfully', async () => {
    ...
    (contacts.getExistingContactByEmail as jest.Mock).mockResolvedValueOnce(null);
